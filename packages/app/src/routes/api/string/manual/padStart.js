...
router.post("/padStart", (req, res) => {
  ...
  if (typeof str !== "string") {
    const result = "Str should be a string";
    res.statusCode = 400;
    res.json({ status: "bad_request", language: "javascript", message: result });
  } else if (
    str.length >= Number(length) ||
    (typeof fill !== "undefined" && String(fill).length === 0) ||
    isNaN(Number(length)) ||
    (str.length === 0 && Number(length) === 0)
  ) {
    ...
    if (typeof fill === "undefined") {
      ...
      str.length + String(fill).length < Number(length) ||
      (str.length === 0 && String(fill).length > 0)
    ) {
      fillNew = String(fill);
    ...
    function increaseFill(fillToIncrease) {
      if (
        String(fillToIncrease).length + str.length < Number(length) ||
        (str.length === 0 && String(fill).length > 0)
      ) {
