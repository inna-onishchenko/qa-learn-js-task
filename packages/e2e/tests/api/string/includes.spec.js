...
test("Both GET /api/string/manual/includes and GET /api/string/builtin/includes responses are ok", async ({
  request,
}) => {
  const req1 = await request.post(`/api/string/manual/includes`, {
    data: { str: reqStr1, search: startWithSearch },
  });
  const req2 = await request.post(`/api/string/builtin/includes`, {
    data: { str: reqStr1, search: startWithSearch },
  });
...
test("Both GET /api/string/builtin/includes and GET /api/string/manual/includes can find search at 0 index of a string", async ({
  request,
}) => {
  const req1 = await request.post(`/api/string/builtin/includes`, {
    data: { str: reqStr1, search: startWithSearch },
  }); // single char search at the beginning of a string
  const req2 = await request.post(`/api/string/manual/includes`, {
    data: { str: reqStr1, search: startWithSearch },
  });
  const req3 = await request.post(`/api/string/builtin/includes`, {
    data: { str: reqStr1, search: emptySearch },
  }); // empty search
  const req4 = await request.post(`/api/string/manual/includes`, {
    data: { str: reqStr1, search: emptySearch },
  });
  const req5 = await request.post(`/api/string/builtin/includes`, {
    data: { str: endWithSearch, search: endWithSearch },
  }); // single char search which is equal to string
  const req6 = await request.post(`/api/string/manual/includes`, {
    data: { str: endWithSearch, search: endWithSearch },
  });
  const resBody1 = await req1.json();
  const resBody2 = await req2.json();
...
test("Both GET /api/string/builtin/includes and GET /api/string/manual/includes can find multiple character search in string", async ({
  request,
}) => {
  const req1 = await request.post(`/api/string/builtin/includes`, {
    data: { str: reqStr1, search: middleCharSearch },
  }); // multiple char search in the middle of a string
  const req2 = await request.post(`/api/string/manual/includes`, {
    data: { str: reqStr1, search: middleCharSearch },
  });
  const resBody1 = await req1.json();
...
test("Both GET /api/string/builtin/includes and GET /api/string/manual/includes can find search on last index of a string", async ({
  request,
}) => {
  const req1 = await request.post(`/api/string/builtin/includes`, {
    data: { str: reqStr1, search: endWithSearch },
  }); // single char search at the end of a string
  const req2 = await request.post(`/api/string/manual/includes`, {
    data: { str: reqStr1, search: endWithSearch },
  });
  const resBody1 = await req1.json();
...
test("Both GET /api/string/builtin/includes and GET /api/string/manual/includes can handle non-included search in a string", async ({
  request,
}) => {
  const req1 = await request.post(`/api/string/builtin/includes`, {
    data: { str: reqStr1, search: nanSearch },
  }); // search is not a string
  const req2 = await request.post(`/api/string/manual/includes`, {
    data: { str: reqStr1, search: nanSearch },
  });
  const req3 = await request.post(`/api/string/builtin/includes`, {
    data: { str: reqStr1, search: numberSearch },
  }); // search is not a string
  const req4 = await request.post(`/api/string/manual/includes`, {
    data: { str: reqStr1, search: numberSearch },
  });
  const req5 = await request.post(`/api/string/builtin/includes`, {
    data: { str: endWithSearch, search: startWithSearch },
  }); // single char search is not included in the string
  const req6 = await request.post(`/api/string/manual/includes`, {
    data: { str: endWithSearch, search: startWithSearch },
  });
  const req7 = await request.post(`/api/string/builtin/includes`, {
    data: { str: reqStr1, search: notIncludedLongSearch },
  }); // long search is not included in the string
  const req8 = await request.post(`/api/string/manual/includes`, {
    data: { str: reqStr1, search: notIncludedLongSearch },
  });
  const req9 = await request.post(`/api/string/builtin/includes`, {
    data: { str: endWithSearch, search: middleCharSearch },
  }); // search is longer than a string
  const req10 = await request.post(`/api/string/manual/includes`, {
    data: { str: endWithSearch, search: middleCharSearch },
  });
  const resBody1 = await req1.json();
  const resBody2 = await req2.json();
...
test("Both GET /api/string/manual/includes and GET /api/string/builtin/includes can handle invalid str parameter", async ({
  request,
}) => {
  const req1 = await request.post(`/api/string/manual/includes`, {
    data: { str: nanStr, search: emptySearch },
  });
  const req2 = await request.post(`/api/string/builtin/includes`, {
    data: { str: nanStr, search: emptySearch },
  });
  const req3 = await request.post(`/api/string/manual/includes`, {
    data: { str: numberStr, search: emptySearch },
  });
  const req4 = await request.post(`/api/string/builtin/includes`, {
    data: { str: numberStr, search: emptySearch },
  });
  const resBody1 = await req1.json();
  const resBody2 = await req2.json();
